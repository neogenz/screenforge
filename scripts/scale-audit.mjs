...
async function ensureServer() {
  const reachable = await fetch(baseURL).then(() => true, () => false)
  ...
  while (Date.now() < deadline) {
    if (await fetch(baseURL).then(() => true, () => false)) return child
...
const readings = await page.evaluate(({ SAMPLES }) => {
  ...
  const boxOf = (el) => {
    ...
    const drawsNothing = style.borderTopWidth === '0px' && style.backgroundColor === 'rgba(0, 0, 0, 0)'
