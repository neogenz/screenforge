...
- **There is no direct path to the data, so the function is the wall.** No table URL, no collection endpoint, no anonymous key that opens a read: a client can only call the functions in `apps/backend/convex/`. That is why no table carries a per-verb policy — the rule lives in `authz.ts`, and `auth.md` is its home.
