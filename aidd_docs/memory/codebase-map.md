...
Paths below are relative to `apps/web/` unless stated otherwise; the repository root holds tooling, `apps/` and `packages/`.
...
- `apps/bridge/`: optional local daemon on 127.0.0.1, one token per capability — assistant copy and App Store publication.
- `apps/mcp/`: the MCP daemon. `src/tools/` is the editor tool surface an agent calls; `src/relay/` is the SSE half the browser connects to.
- `packages/project-format/`: the project contract and the closed store-target catalogue, consumed as TypeScript source rather than a build.
- `scripts/`: export validation, contrast audit (browser-resolved, coss's tokens use `--alpha()`/`color-mix()` that no static parser reads), scale audit (counts values rendered in the populated DOM against coss's own type/height/radius steps), `ui-source-audit.mjs` (`pnpm run audit:ui`: every `components/ui/` file matches what the `@coss` registry serves for it, plus static passes for the pre-coss dependencies and slot-prop convention it replaced), and visual probes, plus the gates the pipeline runs — publication, security headers, deployment configuration, landing, and the MCP live probe. Run from the repository root.
