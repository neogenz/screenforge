...
- Filmstrip tiles are sized from the export target's ratio so the preview shows the real framing; a tile carries its number alone, and the full screen name lives in the tooltip, the context menu and the artboard label on canvas.
