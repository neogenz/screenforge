...
- The closed store target catalogue lives in `packages/project-format/src/dimensions.ts`. A project persists one immutable `target`; board geometry, compatible device/template catalogues, snapshots, release metadata, App Store Connect delivery type, validation, ZIP folder and export all derive from it. Legacy `profileId` projects migrate without coordinate changes.
- Device model identifiers are catalogued by family in `packages/project-format/src/catalog-ids.ts`. The web editor, MCP and AI schemas use that same catalogue, so incompatible frames are rejected rather than merely hidden in the UI.
