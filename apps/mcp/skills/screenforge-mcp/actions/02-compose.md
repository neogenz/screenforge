...
4. **Draft.** Place every layer from the active `W = canvas.width` and `H = canvas.height`, using the ratio-based recipes rather than coordinates copied from another profile. Derive a headline box height from its own font size as `lines × fontSize × 1.2` rather than declaring one.
...
| A user capture given as an absolute path     | `screenforge_add_image` with role `screenshot` puts a profile-compatible device frame carrying it on the board |
