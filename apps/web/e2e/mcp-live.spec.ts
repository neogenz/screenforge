 ...
 *
 * Le démon n'est pas lancé ici : c'est un vrai serveur HTTP tenu par le test,
 * qui joue les quatre routes du relais. Ce n'est pas une facilité — un
 * `page.route` ne sait pas tenir une réponse ouverte, or tout le protocole
 * repose sur un flux SSE que le démon garde en vie. La contrepartie est que le
 * test parle le protocole plutôt que de le simuler : une route renommée dans
 * `apps/mcp` casse ce fichier, ce qui est exactement ce qu'on veut d'un
 * contrat.
 *
 * Le port est tiré au sort et poussé dans le stockage local de la page. En dur
 * sur 4591, ce test aurait parlé au vrai démon de quiconque en développe un sur
 * cette machine — et la machine qui fait tourner cette suite est précisément
 * celle-là.
 */

const TOKEN = 'jeton-de-test-mcp'

interface Answer {
  id: string
  ok: boolean
  result?: { layerIds?: string[]; screenIds?: string[] }
  error?: string
}
...
async function connect(page: Page, relay: Relay): Promise<void> {
