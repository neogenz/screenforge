...
describe('project validation', () => {
  ...
  it.each([
    ['missing field', (candidate: Record<string, unknown>) => { delete candidate.locked }],
    ['invalid opacity', (candidate: Record<string, unknown>) => { candidate.opacity = 1.1 }],
    ['zero dimension', (candidate: Record<string, unknown>) => { candidate.width = 0 }],
    ['unknown discriminant', (candidate: Record<string, unknown>) => { candidate.type = 'background' }],
  ])('rejects %s', (_, mutate) => {
    const candidate = project() as unknown as { screens: Array<{ layers: Record<string, unknown>[] }> }
