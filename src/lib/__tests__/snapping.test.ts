...
describe('computeSnap', () => {
  it('aligns edges and centers within the threshold', () => {
    ...
    expect(computeSnap({ left: 93, top: 94, width: 50, height: 50 }, [target], 8)).toMatchObject({
      dx: 7,
      dy: 6,
    })

    expect(computeSnap({ left: 126, top: 126, width: 50, height: 50 }, [target], 2)).toMatchObject({
      dx: -1,
      dy: -1,
    })
  })
