...
- **The filmstrip has no surface of its own**: unlike the top bar and the drawers it carries no island, no tray, no card. It carries previews, and a preview is already a surface — a container around them stacked strip, tile and preview at three neighbouring lightnesses (light theme's chrome ramp spans 0.9 to 1.0) and spent 26px of canvas framing empty space. The tiles are what floats; the strip is only their scroll box.
- **A screen tile is one object, and only the current one wears the marker**: one button holds the preview and its number, bound by `THUMBNAIL_BADGE_GAP` (6) — the same 6 that binds a label to its control in the panels. Before, the number lived in a second button that existed only to duplicate the first at the pointer, `aria-hidden`, `tabIndex={-1}`, with a hand-extended hit area. State is the number's lime pastille (`marker-fill`), never a ring: the old 2px `--color-marker` halo on a 46px-wide tile was the thickest stroke in the app and read as a highlighter. The pastille is only ever on the current screen — drawn under all ten it stopped being a state and became an ornament, ten pucks each more visible as a shape than the digit it carried. At rest the number sits bare on the stage; its 20px box stays reserved so the row does not jump. The label is the rank, not a zero-padded matricule: `01` reads as a code and filled the pastille with two 11px glyphs where one 14px glyph is read at a glance. 20px is the box's floor, not its width — the project caps at ten screens, and a fixed 20 left `10` touching both edges. The button's `rounded-xl` paints nothing: it shapes the focus ring, which otherwise boxed a rounded preview and a round pastille in a sharp rectangle. The pastille takes the same `--shadow-handle` as the preview and the `+`, and answers hover with `marker-hover` — a flat fill with no edge and no reaction is a highlighter, not a control. The actions handle keeps a 28px hit box, not `hit-44`: on a 46×100 tile a 44 square covered 42% of the preview and took the click meant for the screen, and the full action set is already on the tile's right-click.
- **Reordering is shown, not promised**: dragging a tile parts the row by one `THUMBNAIL_SLOT` and hides the dragged tile, so the strip already holds the shape it will have on drop. Native HTML5 drag, no library: the browser draws the item under the cursor, and the shift always vacates the slot the pointer is over, so `dragover` cannot oscillate between two tiles. `FILMSTRIP_GAP` is declared in `lib/stage.ts` rather than left to `gap-2` because the layout and the shift must read the same number.
