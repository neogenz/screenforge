...
**One write path, all or nothing**: every _multi-step_ mutation goes through `runEditorTransaction(mutate, coalesceKey?)` — it validates the candidate project, commits it or discards it whole, and records exactly one undo step. This is what lets a ten-screenshot batch, an AI plan or a locale substitution be a single operation rather than ten partial ones. Never split one user gesture across two transactions: a failed run must leave nothing behind. This does **not** forbid the editor's own actions: `canvas.store` writes `project.store` through `addLayer`/`updateLayer`/`removeLayer`, which is the intended single-property path (see the coalescing note above). What is forbidden is a _third_ way in — a `useProjectStore.setState` in a feature module, bypassing both.
