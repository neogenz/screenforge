...
| Design system | coss ui (Base UI, `@base-ui/react`), installé par le CLI shadcn depuis le registre `@coss` | v1.7+                 |
| Canvas        | Fabric.js                                                                                  | v7                    |
| State         | Zustand                                                                                    | v5+                   |
...
apps/web/src/
  components/
    ui/                  # coss ui (Base UI), installé par le CLI shadcn depuis le registre
                         # `@coss`, jamais retouché (`pnpm run audit:ui`) : Button, Input,
                         # NumberField, Field, Select, Menu, Dialog, AlertDialog, Popover,
                         # Sheet, Command, Toast, ToggleGroup, Toolbar, Combobox, Empty, Skeleton…
    patterns/            # Compositions ScreenForge sur coss, jamais une primitive maison :
                         # Island, DrawerIsland, PanelSection, PropertyRow, UnitField(Pair),
                         # SliderField, SelectField, Segmented, SwatchButton, AngleControl,
                         # IconButton, Hint, ActionMenu, AnchoredPopover, ConfirmAction,
                         # DialogShell, DialogColumns, StepDialog, AsyncPanel, ProcessingPanel,
                         # CommandPalette, SetupFlow, ShortcutsOverlay, StatusChip, SaveStatus,
                         # NoticeStrip, EmptyStage
  ...
  lib/
    ...
    copy.ts               # State/error copy — dialogs, toasts, Empty and AsyncPanel read from here
  design-system/
    tokens.css            # The one extension over coss: stage, marker, artboard, guide,
                          # shadow-handle, z-*, durations and the sole spring (`ease-settle`)
    motion.css             # Durations/easings as utilities, list/mark/step animations, the
                          # extension's `prefers-reduced-motion` block (index.css carries a
                          # second, narrower one for the coss menu-popup timing override)
    stage.css               # Stage grain, marker-fill/soft, squircle, filmstrip scroll
  types/
...
## Design language (v7, coss ui)

- **coss is the vocabulary**: `src/index.css` exposes the shadcn token contract (`background`, `card`, `popover`, `muted`, `secondary`, `accent`, `primary`, `border`, `input`, `ring`, `destructive`, `success`, `warning`, `info` + `-foreground` pairs), filled by coss's own neutral palette (`@coss/colors-neutral`) instead of a hand-picked ramp. Only what shadcn has no name for sits outside it, in `design-system/tokens.css`: `stage`, `marker*`, `artboard-*`, `selection-soft`, `guide*`, `shadow-handle`, `shadow-handle-focus`, plus six `z-*`. Never re-theme a shadcn name to mean something else, and never edit a file under `components/ui/` by hand — it is installed by the shadcn CLI from the `@coss` registry, and `pnpm run audit:ui` fails naming the file the moment its content drifts from what the registry serves.
...
- **Tokens**: all colors OKLCH in `src/index.css`'s `@theme inline` (light values in bare `:root`, `.dark` overrides — coss's own convention; there is no `.light` class anymore). `design-system/tokens.css` adds its own `@theme inline` block for the sixteen extension tokens above. True neutral, chroma 0 on every chrome surface — a colour-judgement tool must not tint what sits next to the artboard. The light theme still runs on **two levels, not five**: stage `oklch(0.965 0 0)`, cards `oklch(1 0 0)`, and the card's own `shadow-lg/5` does the separating — the number carried over unchanged from the previous palette, because the reasoning (a lighter stage gives a shadow more room to bite) never depended on which system drew the ramp. `--muted-foreground` is coss's one corrected value: its stock dark-theme mix under-shoots the floor `audit:contrast` holds on `card`/`muted`, so `:root.dark` overrides the mix ratio in `tokens.css` rather than the palette import — the one deliberate literal in a file that otherwise only names concepts coss doesn't have.
...
- **Closed scales are coss's own**: type, control heights and radii follow coss's `text-xs/sm/base/lg/xl`, `h-6…h-11` (rendered narrower via `sm:` at desktop width) and `radius-sm/md/lg/xl/2xl` scales rather than a bespoke one. `--radius: 0.625rem` is the single upstream value the radius chain derives from (`calc(var(--radius) ± Npx)`): `sm` 6px, `md` 8px, `lg` 10px (buttons, inputs, `before:` insets), `xl` 14px, `2xl` 18px (islands, popovers, modals). `pnpm run audit:scale` counts the values actually rendered in the populated DOM rather than comparing against a hand-kept whitelist — a literal size in a utility class doesn't show up in `index.css`, so the DOM is the only thing left that can still catch a drift — and it names the offending element.
- **Line-height stays on grid, without a local override**: coss's own type scale already pairs each rendered size with a fixed pixel line-height rather than a ratio, so nothing in the project needs to strip Tailwind's named `leading-tight/snug/relaxed/loose` utilities anymore — the `--leading-*: initial` reset is gone with the palette it belonged to. `pnpm run audit:scale` still measures the grid directly on rendered elements rather than trusting the mechanism: it reads computed line-height in pixels and fails on any value that isn't a multiple of the 4px baseline, wherever it came from. `leading-none` remains in deliberate use on single-line dialog and card titles, where there is no second line to misalign.
- **The floating surface is a component, not a CSS class**: `Island` (`components/patterns/island.tsx`) wraps coss's own `Card` anatomy — `rounded-2xl` (18px), `squircle`, `border`, `bg-popover`, `shadow-lg/5`, `p-1` between frame and control, the same padding coss's own `CardPanel` uses — and every floating surface (top bar, drawers, HUD, selection bar) renders through it. The old v6 surface class and its derived-inset formula (an outer radius minus an inner one, to keep a nested control's curve concentric) are gone: coss's fixed padding convention replaced the geometry, so there is now exactly one definition of the surface instead of the two a CSS class plus component-level overrides used to carry. A drawer passes `Island flush` — its header bleeds to the edge, and `DrawerBody` carries the padding for the content instead.
- **The filmstrip is not an island and has no inset**: its `FILMSTRIP_PADDING` (4) is the clearance its scroll box owes a tile's focus ring (2px stroke, 2px offset), and its top carries `THUMBNAIL_LIFT` (4) on top of that, the room the current tile needs to rise out of the row. Nothing else. `overflow-x: auto` forces the other axis, so a ring or a lift with no room for it would raise a vertical scrollbar — the strip pins `overflow-y: hidden`, the one pairing that does not force, and reserves `FILMSTRIP_SCROLLBAR` (12) in its height. The horizontal bar stays visible and thin: on a narrow window it is what says there are more screens to the right. Reserved permanently rather than when it appears, since twelve transparent pixels on a surfaceless strip are invisible while a height that changed with the screen count would make the stage jump.
...
- **Type**: Inter Variable, self-hosted (`@fontsource-variable/inter`, imported straight into `index.css`) rather than a `<link>` — the old flip on `<link media="print">` that deferred a font stylesheet off the critical path is gone with it, since a bundled variable font has no third-party latency left to hide from first paint. Geist Mono (`@fontsource-variable/geist-mono`) is coss's own choice for `kbd`/`code`, also self-hosted. 14px body, tabular figures for numeric fields, no all-caps labels. Content fonts (text layers on the canvas) still load from Google Fonts on demand — a different concern a self-hosted UI font was never going to cover.
...
- **Primitives are coss's, never hand-rolled**: a feature never builds its own button, input, dialog or scroll area — it composes `components/ui/` (coss, untouched) directly, or a shared shape from `components/patterns/` when coss has no equivalent (a colour swatch, an angle control, a scroll fade). Nothing under `ui/` is edited in place: a one-line change there is a component that no longer updates from the CLI, exactly the state coss replaced. Content default colors live in `src/lib/content-defaults.ts`, never inline hex in components.
- **A coss size variant declares itself twice, so an override must too**: `buttonVariants` writes `h-9 px-… sm:h-8`, and `cn()` (tailwind-merge) indexes its conflict groups by modifier — a bare `h-auto` replaces `h-9` and leaves `sm:h-8` standing. Above 640px, which is every width the editor is used at, the button stays pinned while `items-center` splits the overflow above and below it. A button that must fit its content is therefore written `h-auto sm:h-auto`, never `h-auto` alone. Measured on the project-switcher row: a box of 43px for 60px of content, the name painted over the row above and the availability chip over the one below, in the list whose whole job is to tell two projects apart. Same mechanism on the icons: `[&_svg:not([class*='size-'])]:size-4.5 sm:…:size-4` beats the `width`/`height` attributes Lucide's `size` prop writes, so **only a `size-*` class actually sizes an icon inside a button** — the device tool's chevron, declared `size={9}`, rendered 16 and pushed the pair 2px out of its own 32px hover square on both sides. The rest of the app still declares `size={13}`, `size={12}` and gets 16px; normalising that is a separate arbitration, not a bug fix. `expectNoClippedControl` (`e2e/helpers.ts`) is what holds the line: it sweeps every `[data-slot="button"]` on the page and fails on any child painting more than 1px outside its control, on either axis. It is called from `semantics`, `project-file`, `export` and `ai-campaign` — the switcher one deliberately before that spec's drop to 600px, since below the breakpoint the defect does not exist.
- **Guard-rails**: `pnpm run audit:contrast` reads every token the way a browser resolves it (Chromium, not a regex on `index.css` — coss writes its palette in `--alpha()` and `color-mix()`, which no static parser reads) and fails if any ink/surface pair in the matrix, or any of the closed pairs the matrix can't express (`marker-ink` on `marker`, coss's own `*-foreground` on `primary`/`secondary`/`accent`/`card`, `destructive-foreground` across `card`/`background`/`stage`/`muted`), drops under 4.5:1 in either theme; `pnpm run audit:scale` fails the same way if the rendered type, height, radius or gap scales open up, or a rendered line-height leaves the grid, naming the offending elements; `pnpm run audit:ui` fails naming the file the moment a `components/ui/` primitive's content drifts from what the `@coss` registry serves for it, or the pre-coss dependencies and slot-prop convention it replaced reappear anywhere in `src/`; `pnpm run probe:visual` captures dark/light across empty, populated, an open Export dialog, an open menu, and a screen-less board.
- **L'éditeur et la vitrine ne partagent pas leur thème, et une classe qui traverse ne dit rien**: `landing.css` porte son propre `@theme static`, l'éditeur tient le sien dans `index.css` et `design-system/tokens.css`. Un jeton déclaré d'un seul côté s'écrit sans erreur de l'autre, où Tailwind n'émet rien pour lui — la compilation passe, l'élément se peint à sa taille héritée, et `audit:scale` y lit une valeur parfaitement légitime de l'échelle. Mesuré : `text-2xs` (11px, l'échelon que la vitrine a gardé quand l'éditeur est passé aux tailles coss 12/14/16) écrit dans quatre dialogues, où les six fragments d'une ligne rendaient tous 14px, sans aucune hiérarchie. `audit:ui` relit les `@theme` des deux feuilles à chaque exécution et refuse un jeton de la vitrine dans `src/` hors `landing/`, en nommant le fichier et le jeton d'origine ; les `:root` ordinaires sont hors périmètre, puisqu'y réécrire `--shadow-md` retouche une valeur que Tailwind fournit déjà au lieu de créer une classe. Ce que la garde ne couvre pas : toute classe morte en général. Il y faudrait la CSS de production — en développement Tailwind n'a pas encore vu les primitives des dialogues paresseux, et un balayage de `document.styleSheets` sur serveur froid déclare morte 165 classes coss vivantes.
...
- Never edit a file under `components/ui/` — it is installed by the shadcn CLI from the `@coss` registry (`components.json`, `pnpm dlx shadcn@latest add @coss/<name>`) and `pnpm run audit:ui` fails naming it the moment it drifts from the registry. Project-specific composition lives in `components/patterns/` instead.
- Base UI has no `asChild`/`Slot`: pass a `render` prop (`useRender` + `mergeProps`) to change the rendered tag or merge props onto a caller-supplied element — see `patterns/island.tsx` for the shape.
- Every part of a coss component carries `data-slot="…"` (`button`, `dialog-popup`, `menu-item`…) — style, test and audit against it rather than against a class name or a tag.
- Base UI portals position relative to the nearest stacking context, so the app root needs `isolation: isolate` (already set on `#root`); dropping it silently breaks popover/menu placement.
- Entrances and exits are plain CSS reacting to `data-starting-style`/`data-ending-style`/`data-open` — no animation library. A component kept mounted during its exit needs `keepMounted`.
